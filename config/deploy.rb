set :application, "js-la-jobs"
set :application_port, 5000
...
namespace :deploy do
  ...
  script
      ...
      exec sudo -u #{admin_runner} sh -c "LOG_PATH=#{log_path} APP_HOST=#{app_host} NODE_ENV=#{node_env} PORT=#{application_port} #{application_binary} #{current_path}/#{node_file}"
  end script
  respawn
UPSTART
